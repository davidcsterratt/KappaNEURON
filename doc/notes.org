...
** TODO =multiCompartmentReaction._get_memb_flux()= 

- Gets flux across the membrane due to univalent ion in mA/cm^2
- *Returns*: =self._memb_scales*rates= where =rates= comes from
  =_evaluate()= and is in molecules/µm^2/ms and =self._memb_scales= is set in
  =multiCompartmentReaction._do_memb_scales()= as =-area_ratios *
  FARADAY / (10000 * molecules_per_mM_um3)= where =area_ratios=  is
  normally 1. 
...
** TODO =_update_indices()=
** TODO =generalisedReaction._setup_membrane_fluxes(node_indices, cur_map)=
...
** TODO =self._memb_scales=

** TODO =self._mult=

Set in =_update_indices()=, its value depends on the value of
